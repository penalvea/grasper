...
#define GRIPPER_MANIPULATION_VALVE 0.5
#define GRIPPER_MANIPULATION_HOTSTAB 0.8
#define GRIPPER_OPENED 1.0

#define CURRENTTHRESHOLD_VALVE 1.0
...
int main(int argc, char **argv){
	...
	VirtualImage g(nh,"/stereo_down/left/image_color","/stereo_down/left/camera_info");
	...
	robot.getJointValues(current_joints);
	while(current_joints[4]<GRIPPER_MANIPULATION_VALVE && ros::ok()){
		vpDisplay::display(Ic);
		vpDisplay::displayFrame(Ic,cMe, g.K, 0.1, vpColor::none, 2);
		vpDisplay::flush(Ic);
	...
	while((current_joints[3]<2) && ros::ok() && robot.getCurrent()<CURRENTTHRESHOLD_VALVE){
		robot.setJointVelocity(vel);
		ros::spinOnce();
		robot.getJointValues(current_joints);
	}
	...
	sleep(1);
	while(robot.getCurrent()>CURRENTTHRESHOLD_VALVE)
	{
		ros::spinOnce();
	}
	...
	while((current_joints[3]>-0.5) && ros::ok() && robot.getCurrent()<CURRENTTHRESHOLD_VALVE){
		robot.setJointVelocity(vel);
		ros::spinOnce();
		robot.getJointValues(current_joints);
	}
	...
	robot.getJointValues(current_joints);
		...
		while((current_joints[3]<0.0) && ros::ok() && robot.getCurrent()<CURRENTTHRESHOLD_VALVE){
			robot.setJointVelocity(vel);
			ros::spinOnce();
			robot.getJointValues(current_joints);
		}
